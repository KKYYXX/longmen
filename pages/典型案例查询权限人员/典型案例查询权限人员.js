...
Page({
  ...
  fetchRecords() {
    // 调用后端 /user/query_model接口获取项目查询权限人员列表
    wx.request({
      url: apiConfig.buildUrl('/app/user/query_model'), // 直接调用后端接口
      method: 'GET',
      success: res => {
  ...
  confirmAdd() {
    ...
    wx.request({
      ...
      url: apiConfig.buildUrl('/app/user/query_model_add'),
      method: 'POST',
      success: res => {
        if (res.statusCode === 200) {
    ...
    wx.showModal({
      ...
      success: res => {
        if (res.confirm) {
          ...
          wx.request({
            url: apiConfig.buildUrl('/app/user/query_model_delete'),
            method: 'POST',
            header: { 
              'content-type': 'application/x-www-form-urlencoded' 
            },
