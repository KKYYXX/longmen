...
Page({
  ...
  fetchRecords() {
    // 调用后端 /user/query_zc 接口获取项目查询权限人员列表
    wx.request({
      url: apiConfig.buildUrl('/app/user/query_zc'), // 直接调用后端接口
      method: 'GET',
      success: res => {
  ...
  confirmAdd() {
    ...
    wx.request({
      ...
      url: apiConfig.buildUrl('/app/user/query_zc_add'),
      method: 'POST',
      success: res => {
        if (res.statusCode === 200) {
    ...
    wx.showModal({
      ...
      success: res => {
        if (res.confirm) {
          ...
          wx.request({
            url: apiConfig.buildUrl('/app/user/query_zc_delete'),
            method: 'POST',
            header: { 
              'content-type': 'application/x-www-form-urlencoded' 
            },
