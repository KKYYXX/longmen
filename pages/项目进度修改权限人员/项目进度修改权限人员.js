...
Page({
  ...
  fetchRecords() {
    ...
    wx.request({
      url: apiConfig.buildUrl('/app/user/alter_progress'), // 直接调用后端接口
      method: 'GET',
      success: res => {
  ...
  confirmAdd() {
    ...
    wx.request({
      ...
      url: apiConfig.buildUrl('/app/user/alter_progress_add'),
      method: 'POST',
      success: res => {
        if (res.statusCode === 200) {
    ...
    wx.showModal({
      ...
      success: res => {
        if (res.confirm) {
          ...
          wx.request({
            url: apiConfig.buildUrl('/app/user/alter_progress_delete'),
            method: 'POST',
            header: { 
              'content-type': 'application/x-www-form-urlencoded' 
            },
