Page({
  ...
  fetchRecords() {
    // 调用后端 /user/alter_progress 接口获取项目进度修改权限人员列表
    wx.request({
      url: 'http://127.0.0.1:5000/app/user/alter_progress', // 直接调用后端接口
      method: 'GET',
      success: res => {
  ...
  confirmAdd() {
    ...
    wx.request({
      ...
      url: 'http://127.0.0.1:5000/app/user/alter_progress_add',
      method: 'POST',
      success: res => {
        if (res.statusCode === 200) {
    ...
    wx.showModal({
      ...
      success: res => {
        if (res.confirm) {
          ...
          wx.request({
            url: 'http://127.0.0.1:5000/app/user/alter_progress_delete',
            method: 'POST',
            header: { 
              'content-type': 'application/x-www-form-urlencoded' 
            },
