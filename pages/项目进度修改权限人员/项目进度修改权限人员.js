Page({
  ...
  fetchRecords() {
    ...
    wx.request({
      url: 'http://127.0.0.1:5000/app/user/alter_progress', // 👈 替换成你的接口
      method: 'GET',
      success: res => {
        if (res.data.success) {
  ...
  confirmAdd() {
    ...
    // 模拟后端保存逻辑
    wx.request({
      url: 'http://127.0.0.1:5000/app/user/alter_progress_add', // 👈 替换成你的新增接口
      method: 'POST',
  ...
  onDeleteRecord(e) {
    ...
    wx.showModal({
      ...
      success: res => {
        if (res.confirm) {
          wx.request({
            url: 'http://127.0.0.1:5000/app/user/alter_progress_delete', // 👈 替换成你的删除接口
            method: 'POST',
            header: { 'content-type': 'application/json' },
